namespace Nettle.Compiler
{
    public sealed class NettleCompiler : INettleCompiler
    {
        ...
        public Func<object, string> Compile(string templateContent)
        {
        ...
        {
            string template(object model) => _renderer.Render(parsedTemplate, model);
        ...
        public void AutoRegisterViews(string directoryPath)
        {
            Validate.IsNotEmpty(directoryPath);

            var matchingViews = ViewReader.ReadAll(directoryPath);
