...
internal sealed class FunctionParser : NettleParser, IBlockParser
{
    ...
    public CodeBlock Parse(ref string templateContent, ref int positionOffSet, string signature)
    {
        ...
        {
            var errorPosition = (positionOffSet + (signature.Length - 1));
        ...
        var name = body.Crop(0, body.IndexOf('(') - 1);
        ...
        var parameterSegment = body.Substring(parameterStart + 1, parameterLength);

        // Tokenize the functions parameters so we can parse them
        if (false == String.IsNullOrEmpty(parameterSegment))
        {
            var tokenizer = new Tokenizer(',');
            var parameterValues = tokenizer.Tokenize(parameterSegment);
            ...
            foreach (var token in parameterValues)
            {
                ...
                {
                    throw new NettleParseException("Parameter values cannot be empty.", positionOffSet);
                }

                var type = ResolveType(token);
                var value = type.ParseValue(token);

                parameters.Add(new FunctionCallParameter(token, value, type));
            }
        }
        ...
        return new FunctionCall(signature, name)
        {
