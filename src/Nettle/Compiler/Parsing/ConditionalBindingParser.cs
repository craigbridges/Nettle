...
internal sealed class ConditionalBindingParser : NettleParser, IBlockParser
{
    ...
    public bool Matches(string signatureBody)
    {
    ...
    {
        var body = UnwrapSignatureBody(signature)[1..].Trim();
        ...
        if (tokens.Length != 5 || tokens[1] != "?" || tokens[3] != ":")
        {
            throw new NettleParseException
            (
                $"The syntax for the condition '{body}' is invalid.",
                positionOffSet
            );
        }
        ...
        var conditionExpression = _expressionParser.Parse(conditionSignature);
        ...
        return new ConditionalBinding(signature, conditionExpression)
        {
